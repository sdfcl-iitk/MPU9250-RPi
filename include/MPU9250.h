...
private:
    void initMPU9250() {
        ...
        ::sleep(100 / 1000.0);                                  // Wait for all registers to reset
    ...
    void initAK8963() {
        ...
        if (b_verbose) {
            ...
            printf("X-Axis sensitivity offset value %.3f\n", mag_bias_factory[0]);
            printf("Y-Axis sensitivity offset value %.3f\n", mag_bias_factory[1]);
            printf("Z-Axis sensitivity offset value %.3f\n", mag_bias_factory[2]);
        }
    }
...
private:
    ...
    void set_acc_gyro_to_calibration() {
        ...
        ::sleep(40 / 1000.0);                                  // accumulate 40 samples in 40 milliseconds = 480 bytes
    }
    ...
    void calibrate_mag_impl() {
        ...
        if (b_verbose) {
            ...
            printf("AK8963 mag biases (mG) %.3f, %.3f, %.3f\n", mag_bias[0], mag_bias[1], mag_bias[2]);
            printf("AK8963 mag scale (mG) %.3f, %.3f, %.3f\n", mag_scale[0], mag_scale[1], mag_scale[2]);
        }
    ...
    void collect_mag_data_to(float* m_bias, float* m_scale) {
        ...
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            ...
            if (MAG_MODE == 0x02) ::sleep(135 / 1000.0);  // at 8 Hz ODR, new mag data is available every 125 ms
            if (MAG_MODE == 0x06) ::sleep(12 / 1000.0);   // at 100 Hz ODR, new mag data is available every 10 ms
        }
    ...
    bool self_test_impl()  // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        ::sleep(25 / 1000.0);                                     // Delay a while to let the device stabilize
        ...
        if (b_verbose) {
            printf("x-axis self test: acceleration trim within : %.3f%% of factory value\n", self_test_result[0]);
            printf("y-axis self test: acceleration trim within : %.3f%% of factory value\n", self_test_result[1]);
            printf("z-axis self test: acceleration trim within : %.3f%% of factory value\n", self_test_result[2]);
            
            printf("x-axis self test: gyration trim within : %.3f%% of factory value\n", self_test_result[3]);
            printf("y-axis self test: gyration trim within : %.3f%% of factory value\n", self_test_result[4]);
            printf("z-axis self test: gyration trim within : %.3f%% of factory value\n", self_test_result[5]);
        }
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress) {
        ...
        return i2c_smbus_read_byte_data(i2c_bus, subAddress);
    }

    void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t* dest) {
        ioctl(i2c_bus, I2C_SLAVE, address);
        i2c_smbus_read_i2c_block_data(i2c_bus, subAddress, count, dest);
    }

    void write_byte(uint8_t address, uint8_t subAddress, uint8_t data) {
        ioctl(i2c_bus, I2C_SLAVE, address);
        i2c_smbus_write_byte_data(i2c_bus, subAddress, data);
    }
